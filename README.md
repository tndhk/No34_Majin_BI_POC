...
- **プロンプトパラメータのカスタマイズ (NEW)**:
    - 重視したいKPIや分析の切り口を自由に指定できるオプション入力欄を追加しました。
    - ユーザーが指定したカスタム分析パラメータは自動的にAIプロンプトに反映され、より目的に合った分析設計が可能になります。
    - 例: 「売上高と利益率の関係性を重点的に分析」「地域別・時系列での傾向把握」などを指定できます。
- **グラフのカスタマイズ・編集機能**:
...
2.  **生成フロー**
    ...
    - **Step 2**: (オプション) カスタム分析パラメータ入力欄に、重視したいKPIや分析の切り口を指定できます。
    - **Step 2**: 「🚀 Generate Blueprint」をクリックし、AIが提案するグラフ構成案を確認します。
    - **Step 3**: 内容に問題なければ「✨ Generate & Visualize」をクリックします。
    - **完了**: 生成されたHTMLダッシュボードが画面に表示されます。ダウンロードも可能です。
